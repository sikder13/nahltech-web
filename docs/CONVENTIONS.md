# nahltech-web — Project Conventions
...
11. Commit messages: conventional commits, written as the project author.
    No tool attribution of any kind — no "Generated with" footers, no
    co-author trailers for tools, no tool names — in commits, PR titles,
    PR descriptions, code comments, or file headers.
...
13. Visual verification: end every UI session with the site viewable —
    a running dev server or a deployment URL — plus the list of URLs to
    review and a one-line note on what changed at each.
...
acceptance criteria in the task brief verified and stated back.

## Reference

Full architecture: ARCH-1 (docs/ARCH-1-system-architecture.md).
