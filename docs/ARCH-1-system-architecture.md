...
- **`next/image` has one documented exception: the header logo.** Content imagery goes through `next/image`, as above. The brand mark does not — it is a fixed 24px asset on every page, pre-optimised to a 5 KB WebP by `npm run build:logo`, and `next/image`'s client runtime measured **+5.0 kB gz on every page**, which put first-load over the ceiling to optimise something already optimised. It ships as a plain `<img>` with explicit `width`/`height`, so it still reserves its own space and contributes nothing to CLS. This exception is for fixed-size brand assets only; the moment real photography lands (team photos on `/about`), that goes through `next/image`.
