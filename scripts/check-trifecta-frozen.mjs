 ...
 * Trifecta is frozen.
 *
 * The Trifecta dashboard shipped with a printed letter, and that letter
 * describes this page as it is. Template 2 stays a living template for new
 * companies, so Trifecta has no byte pins; this rendered-page gate is what
 * holds it still while the code around it grows. This check runs in CI
 * after `next build` and compares the built Trifecta page with a golden copy
 ...
 * it starts.
 *
 * Usage: npm run build && node scripts/check-trifecta-frozen.mjs
 * To refresh the golden file after a deliberate, founder-approved change to
 * Trifecta: node scripts/check-trifecta-frozen.mjs --write
 */
import { readFileSync, writeFileSync } from "node:fs";

const TOKEN = "trifecta-medical-llc-e15c170849";
const BUILT = `.next/server/app/m2/${TOKEN}.html`;
const GOLDEN = "tests/golden/trifecta-body.html";
...
if (built === golden) {
  console.log(
    `Trifecta is unchanged: the built page matches ${GOLDEN} exactly.`,
  );
  process.exit(0);
}
...
console.error(
  "Trifecta has changed. It is frozen: it shipped with a printed letter.",
);
