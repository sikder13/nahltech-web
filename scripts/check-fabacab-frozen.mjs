 ...
 * FabACab is frozen.
 *
 * The FabACab dashboard shipped with a printed letter, and that letter
 * describes this page as it is. Template 2 stays a living template for new
 * companies, so FabACab has no byte pins; this rendered-page gate is what
 * holds it still while the code around it grows. This check runs in CI
 * after `next build` and compares the built FabACab page with a golden copy
 ...
 * it starts.
 *
 * Usage: npm run build && node scripts/check-fabacab-frozen.mjs
 * To refresh the golden file after a deliberate, founder-approved change to
 * FabACab: node scripts/check-fabacab-frozen.mjs --write
 */
import { readFileSync, writeFileSync } from "node:fs";

const TOKEN = "fabacab-06ce93330a";
const BUILT = `.next/server/app/m2/${TOKEN}.html`;
const GOLDEN = "tests/golden/fabacab-body.html";
...
if (built === golden) {
  console.log(
    `FabACab is unchanged: the built page matches ${GOLDEN} exactly.`,
  );
  process.exit(0);
}
...
console.error(
  "FabACab has changed. It is frozen: it shipped with a printed letter.",
);
