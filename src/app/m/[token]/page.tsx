...
export default async function DashboardPage({ params }: Params) {
  ...
  if (!config) notFound();
  const shared = sharedCopy();
  const { company, model, market, findings, proposal } = config;
  ...
  return (
    <>
      <main className="mx-auto max-w-(--container-measure) px-sm pt-lg pb-2xl lg:max-w-(--container-page) lg:px-md">
        ...
        {/* 4 · The public series */}
        <section aria-labelledby="market-heading" className="mt-2xl">
        ...
        <section aria-labelledby="findings-heading" className="mt-2xl">
        ...
        <section aria-labelledby="proposal-heading" className="mt-2xl">
                ...
                <table className="w-full border-collapse text-xs tabular-nums sm:text-sm">
                  <thead>
                    <tr className="border-b border-border text-start text-text">
                      {proposal.ledger.columns.map((c) => (
                        <th
                          key={c}
                          scope="col"
                          className="py-2xs pe-xs text-start font-semibold last:pe-0 sm:pe-sm"
                        >
                          {c}
                        </th>
                      ))}
                      ...
                      <Fragment key={row.cells.join("|")}>
                        ...
                        >
                          {row.cells.map((cell, i) => (
                            <td
                              key={`${i}-${cell}`}
                              className="py-2xs pe-xs whitespace-nowrap last:pe-0 sm:pe-sm"
                            >
                              {cell}
                            </td>
                          ))}
        ...
        {/* 7 · The ask */}
        <section aria-labelledby="book-heading" className="mt-2xl">
