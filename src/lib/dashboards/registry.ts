...
/**
 * Loads prospect dashboards from `content/dashboards/`.
 *
...
const DIR = path.join(process.cwd(), "content", "dashboards");
...
export function allDashboards(): DashboardConfig[] {
  ...
  const loaded = files.map((file) => {
    ...
    if (!parsed.success) {
      throw new Error(
        `content/dashboards/${file} is invalid: ${parsed.error.issues
...
/**
 * `/eckco` → `/m/<token>`. Temporary (307), never permanent: the letter
 * prints the short address, and the token scheme behind it must stay free
 * to change without browsers having cached the old target forever.
 */
export function dashboardRedirects() {
  return allDashboards().map((d) => ({
    source: `/${d.slug}`,
    destination: `/m/${d.token}`,
    permanent: false,
  }));
}
