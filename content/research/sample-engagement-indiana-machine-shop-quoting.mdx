...
The pattern in this walkthrough repeats across every job shop we have studied: the constraint is not the machines and it is not the people. It is that the shop's accumulated knowledge — every part it has ever quoted and run — sits in folders where only one overworked expert can reach it. The build gives that memory back to the business.
